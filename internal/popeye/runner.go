...
type popeyeJSON struct {
	Popeye struct {
		...
		Grade      string `json:"grade"`
		Sanitizers []struct {
			Sanitizer string `json:"sanitizer"` // resource type: "pod", "svc", etc.
			Tally     struct {
				OK   int `json:"ok"`
				Info int `json:"info"`
...
func (r *Runner) Run(ctx context.Context) (*Report, error) {
	...
	args := []string{"--output", "json", "--force-exit-zero"}
	if r.kubeconfig != "" {
	...
	r.logger.InfoContext(ctx, "running popeye scan",
	...
	findingID := 0
	for _, san := range raw.Popeye.Sanitizers {
		...
		for resourceName, issues := range san.Issues {
			...
			for _, issue := range issues {
				...
				f.Explanation = explainFinding(san.Sanitizer, issue.Message, sev)
				f.Fix = suggestFix(san.Sanitizer, issue.Message, ns, name)
				f.Command = suggestCommand(san.Sanitizer, issue.Message, ns, name)
