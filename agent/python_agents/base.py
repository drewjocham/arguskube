...
from .models import AgentResult, ChatMessage, ProactiveInsight, SessionContext
...
class BaseAgent(ABC):
    ...
    def emit(self, event_type: str, payload: dict[str, Any] | None = None) -> AgentEvent:
        """Emit a typed event through the agent's EventBus.

        If no bus is attached (e.g. agent used standalone), the event is
        created but goes nowhere — no crash, no side effect.
        """
        event = AgentEvent(
    ...
    def _model_to_json_schema(self, model: type[BaseModel]) -> dict[str, Any]:
        return json.loads(model.model_json_schema())
