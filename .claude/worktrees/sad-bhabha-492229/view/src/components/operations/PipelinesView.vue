    ...
    <div class="body">
      ...
      <div v-else-if="activeTab === 'prs'" class="tab-pane">
        <div class="empty">
          ...
          <div class="empty-body">
            When wired up, this view lists open PRs from <strong>{{ providerLabel }}</strong>
            with author, branch, status and a side panel showing diff &amp; CI checks. From a PR
            you'll be able to run AI actions:
          </div>
          <div class="ai-actions preview" aria-hidden="true">
            <button class="action-btn primary" disabled>✨ Summarize PR</button>
            <button class="action-btn" disabled>🔍 Code Review</button>
            <span class="preview-note">Available once provider integration is wired up.</span>
          </div>
        </div>
      </div>
      ...
      <div v-else-if="activeTab === 'branches'" class="tab-pane">
        <div class="empty">
          <div class="empty-title">No branches loaded</div>
          <div class="empty-body">
            When wired up, this view lists branches from <strong>{{ providerLabel }}</strong>
            along with their last commit, ahead/behind counts vs. the default branch, and the
