...
const agentNamespace = ref('argus')
...
function deployToolKey(name) {
  ...
  if (name === 'argus-agent') return 'argus-agent'
  if (name === 'docker') return 'argus-stack'
...
function toolDescription(name) {
  const descriptions = {
    ...
    'argus-agent': 'In-cluster DaemonSet that provides real-time anomaly detection, topology mapping, and streaming metrics',
  }
  return descriptions[name] || ''
}

function isInstallable(name) {
  return name === 'popeye' || name === 'argus-agent'
}

async function handleInstall(name) {
  if (name === 'argusScan') {
    lastResult.value = await installArgusScan()
  } else if (name === 'argus-agent') {
...
<template>
  <div class="setup-view">
    <div class="setup-header">
      ...
      <div class="setup-subtitle">One-click setup for Argus tools and services</div>
    </div>
    ...
    <div class="tool-grid">
      <div v-for="tool in tools" :key="tool.name" class="tool-card" :class="{ installed: tool.installed }">
        <div class="tool-main">
          ...
          <div class="tool-actions">
            <template v-if="isInstallable(tool.name)">
              ...
              <button
                v-else-if="tool.name === 'argus-agent'"
                class="remove-btn"
                :disabled="actionLoading === tool.name"
        ...
        <div v-if="tool.name === 'argus-agent' && !tool.installed" class="agent-config">
          <label class="config-label">Deploy namespace:</label>
          <input type="text" class="config-input" v-model="agentNamespace" placeholder="argus" />
        </div>
    ...
    <div class="info-section">
      ...
      <div class="info-body">
        The Argus Agent is deployed as a DaemonSet with read-only RBAC permissions.
