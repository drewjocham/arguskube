...
describe('CenterPanel.vue — Integration', () => {
  ...
  it('renders the alerts route with both Cluster Alerts and Watchers tabs', () => {
    const wrapper = createWrapper({ activeNav: 'alerts' })
    // The alerts route has two sub-tabs: "Cluster Alerts" (the legacy
    // widget overview) and "Watchers & Silences" (the dedicated
    // AlertsView). The default tab is owned by the CenterPanel and
    // controlled by alertsTab — both labels must be visible regardless.
  ...
  // Topology is no longer a top-level monitoring view — it lives as an
  // optional widget inside the alerts overview. This test is kept to
  // document the removal; the assertion is satisfied by the fact that
  // activeNav='topology' falls through to no specific renderer.
  it('activeNav="topology" no longer routes to its own page (widget-only)', () => {
    const wrapper = createWrapper({ activeNav: 'topology' })
    expect(wrapper.find('.mock-topology-map').exists()).toBe(false)
  })
