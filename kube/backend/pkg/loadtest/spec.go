...
const (
	...
	PayloadKindAI       PayloadKind = "ai"       // body generated by the LLM
	PayloadKindFile     PayloadKind = "file"     // body read from a local file/dir
)
...
type Payload struct {
	...
	Size int `json:"size"`
	// SamplePool, when set, holds a fixed pool of payloads the engine
	// rotates through (one body per Publish, round-robin). Used by
	// PayloadKindFile in "exact" mode where the user pointed at a
	// directory of varied messages. Empty → engine sends Bytes for
	// every message (the default). Excluded from JSON so the record
	// doesn't balloon with N copies of every file.
	SamplePool [][]byte `json:"-"`
}
...
func (s *RunSpec) Validate() error {
	...
	if len(s.Payload.Bytes) == 0 && len(s.Payload.SamplePool) == 0 {
