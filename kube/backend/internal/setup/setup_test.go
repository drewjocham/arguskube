...
func TestNewManager(t *testing.T) {
	...
	mgr := setup.NewManager("", "default", "argus", logger)
	if mgr == nil {
...
func TestCheckAllToolsReturnsAllExpectedTools(t *testing.T) {
	...
	expectedNames := []string{"popeye", "docker", "helm", "argus-agent", "kubectl"}
...
func TestKubectlArgsWithConfig(t *testing.T) {
	logger := slog.New(slog.DiscardHandler)
	mgr := setup.NewManager("/home/user/.kube/config", "prod-cluster", "argus", logger)
	_ = mgr.CheckAllTools(context.Background())
...
func TestCheckAgentNoKubectl(t *testing.T) {
	...
	t.Error("argus-agent not found in tool statuses")
}
...
func TestAgentDeployOutputFormat(t *testing.T) {
	...
	if !strings.Contains(result.Message, "argus") && result.Success {
