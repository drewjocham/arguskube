...
func (m *Manager) checkAgent(ctx context.Context) ToolStatus {
	// Check if the Argus agent DaemonSet is running in the cluster.
	args := m.kubectlArgs("get", "daemonset", "-l", "app.kubernetes.io/name=argus-agent", "--all-namespaces", "-o", "jsonpath={.items[0].status.numberReady}")
	out := runQuiet(ctx, "kubectl", args...)
	if out != "" && out != "0" {
		return ToolStatus{Name: "argus-agent", Installed: true, Version: out + " ready", Via: "helm", Message: "Agent running in cluster"}
	}
	return ToolStatus{Name: "argus-agent", Installed: false, Message: "Agent not deployed to cluster"}
}
...
// DeployAgent deploys the Argus agent to the cluster using kubectl apply with inline manifests.
func (m *Manager) DeployAgent(ctx context.Context, namespace string) *SetupResult {
	m.logger.InfoContext(ctx, "deploying argus agent", slog.String("namespace", namespace))
...
metadata:
  ...
  namespace: %[1]s
  labels:
    app.kubernetes.io/name: argus-agent
---
  ...
  template:
    metadata:
      labels:
        app.kubernetes.io/name: argus-agent
    spec:
      serviceAccountName: argus-agent
      securityContext:
        runAsNonRoot: true
        runAsUser: 1000
      containers:
      - name: agent
        image: ghcr.io/drewjocham/argus-agent:latest
