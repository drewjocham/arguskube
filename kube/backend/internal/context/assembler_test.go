...
import (
	...
	context2 "github.com/argues/argus/internal/context"
...
func TestNewAssembler(t *testing.T) {
	...
	a := context2.NewAssembler(cfg, gate, detector, logger)
	if a == nil {
...
func TestNewAssemblerNilLogger(t *testing.T) {
	a := context2.NewAssembler(nil, features.NewGate(&config.OnlineDataConfig{}), &mockDetector{}, nil)
	if a != nil {
...
func TestAssemble(t *testing.T) {
	...
	a := context2.NewAssembler(nil, features.NewGate(&config.OnlineDataConfig{}), detector, slog.New(slog.DiscardHandler))
	if a == nil {
		t.Fatal("NewAssembler() returned nil")
	}
