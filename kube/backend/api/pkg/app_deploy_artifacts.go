...
type DeployArtifact struct {
	Tool        string `json:"tool"`   // matches setup tool keys: kubectl, helm, popeye, argus-agent…
...
func deployArtifacts() []DeployArtifact {
	return []DeployArtifact{
		// ── Argus Agent — Helm ───────────────────────────────
		{
			Tool:        "argus-agent",
			Flavor:      "helm",
			Description: "Install the Argus in-cluster agent via the bundled Helm chart.",
			CommandText: `helm upgrade --install argus-agent ./deploy/helm/argus-agent \
  --namespace argus --create-namespace \
  --set image.repository=ghcr.io/drewjocham/argus-agent \
  --set image.tag=v1.0.0`,
			FileName: "argus-agent.values.yaml",
			FileText: `# argus-agent values overrides
image:
  repository: ghcr.io/drewjocham/argus-agent
  tag: v1.0.0
env:
...
`,
  ...
  derailed/popeye:v0.22.0 -A`,
		...
		// ── Monitoring stack — Helm ────────────────────────────────
		{
			...
			CommandText: `helm upgrade --install argus-monitoring ./deploy/helm/argus-monitoring \
  --namespace argus --create-namespace \
  --set kube-prometheus-stack.grafana.adminPassword=$GF_SECURITY_ADMIN_PASSWORD`,
			FileName: "argus-monitoring.values.yaml",
...
`,
		...
		// ── docker-compose stack ───────────────────────────────────
		{
			...
			CommandText: `docker compose -f docker-compose.argus.yaml up -d`,
			FileName:    "docker-compose.argus.yaml",
			FileText: `# Local docker-compose stack for Argus.
# Pre-flight:
#   1. cp .env.example .env  and fill in the secrets
#   2. docker compose -f docker-compose.argus.yaml up -d
services:
  argus-backend:
  ...
  argus-frontend:
    image: ghcr.io/drewjocham/argus-frontend:v1.0.0
    ports: ["8081:8080"]
    environment:
      BACKEND_URL: http://argus-backend:8080
    depends_on: [argus-backend]
  prometheus:
