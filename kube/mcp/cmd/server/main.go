...
import (
	...
	"github.com/argues/argus/mcp/api"
	...
	"github.com/argues/argus/pkg/logging"
)
...
func main() {
	...
	if cfg.showVersion {
		fmt.Printf("argus v%s (commit: %s, built: %s)\n", version, gitCommit, buildDate)
		return
	}
...
func parseFlags() config {
	...
	defaultDB := filepath.Join(home, ".argus", "history.db")
	...
	flag.Usage = func() {
		fmt.Fprintf(os.Stderr, "argus v%s\n", version)
		fmt.Fprintf(os.Stderr, "Usage: argus [options]\n\nOptions:\n")
		flag.PrintDefaults()
	}
...
func executeAction(ctx context.Context, s *server.MCPServer, cfg config, logger *slog.Logger) {
	...
	default:
		logger.Info("starting argus server", "version", version)
		if cfg.httpAddr != "" {
