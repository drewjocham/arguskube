# KubeWatcher API Specification

> Version 0.1.0 — Generated 2026-05-02

KubeWatcher exposes two API surfaces: **Wails bindings** for the native desktop app and an **HTTP/JSON API** for the SaaS web client. Both surfaces call the same Go methods on the `App` struct. All methods return `(result, error)` unless noted otherwise.
...
CORS is enabled for all origins. The server listens on `:8080` by default (`KUBEWATCHER_PORT`).
...
Bidirectional channel for in-cluster KubeWatcher agents. Handled by `Hub.HandleTunnel()`.
...
| `deep-link` | `string` | Custom URL scheme received (`kubewatcher://...`) |
...
|----------|---------|-------------|
| `KUBEWATCHER_KUBECONFIG` | `$KUBECONFIG` | Kubeconfig path (colon-separated for multi-file) |
| `KUBEWATCHER_CONTEXT` | _(current)_ | Override kubeconfig context |
| `KUBEWATCHER_NAMESPACE` | _(all)_ | Default namespace filter |
| `KUBEWATCHER_IN_CLUSTER` | `false` | Use in-cluster config |
...
| `KUBEWATCHER_TIER` | `pro` | Feature tier (`free` / `pro`) |
...
| `KUBEWATCHER_POPEYE_BIN` | `popeye` | Path to popeye binary |
| `KUBEWATCHER_S3_BUCKET` | — | S3 bucket for notebooks |
| `KUBEWATCHER_S3_REGION` | `us-east-1` | S3 region |
| `KUBEWATCHER_S3_ENDPOINT` | — | S3 endpoint (for MinIO etc.) |
| `KUBEWATCHER_S3_ACCESS_KEY` | — | S3 access key |
| `KUBEWATCHER_S3_SECRET_KEY` | — | S3 secret key |
| `KUBEWATCHER_DECISION_LOG` | `DECISION_LOG.md` | Path to decision log |
