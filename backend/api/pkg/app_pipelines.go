...
// $HOME/.kubewatcher/pr-guidelines/<provider>.md.
...
func prGuidelinesPath(provider string) (string, error) {
	...
	dir := filepath.Join(os.ExpandEnv("$HOME"), ".kubewatcher", "pr-guidelines")
...
// --- Code review reports ---
//
// Reports live at $HOME/.kubewatcher/code-reviews/<provider>/<id>.md, where
...
func codeReviewDir(provider string) (string, error) {
	...
	dir := filepath.Join(os.ExpandEnv("$HOME"), ".kubewatcher", "code-reviews", provider)
	if err := os.MkdirAll(dir, 0750); err != nil {
