...
const (
	agentLabelSelector = "app.kubernetes.io/name=kubewatcher-agent"
...
// Connector manages a port-forward to the kubewatcher-agent pod.
...
// findAgentPod locates a running kubewatcher-agent pod.
func (c *Connector) findAgentPod(ctx context.Context, namespace string) (*corev1.Pod, error) {
	...
	if len(pods.Items) == 0 {
		return nil, fmt.Errorf("no kubewatcher-agent pod found in namespace %s", ns)
	}
