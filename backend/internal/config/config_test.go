...
func TestEnvOverrides(t *testing.T) {
	...
	os.Setenv("KUBEWATCHER_PORT", "3000")
	os.Setenv("KUBEWATCHER_LOG_LEVEL", "debug")
	os.Setenv("KUBEWATCHER_LOG_FORMAT", "json")
	os.Setenv("KUBEWATCHER_TIER", "free")
	os.Setenv("DEEPSEEK_API_KEY", "sk-test-key")
	os.Setenv("KUBEWATCHER_CONTEXT", "my-cluster")
	os.Setenv("KUBEWATCHER_IN_CLUSTER", "true")
	os.Setenv("KUBEWATCHER_S3_BUCKET", "my-bucket")
	os.Setenv("ARGOCD_INSECURE", "true")
	os.Setenv("KUBEWATCHER_TRIVY_BIN", "/usr/local/bin/trivy")
...
func TestConfigValidation(t *testing.T) {
	t.Run("rejects both kubeconfig and in-cluster", func(t *testing.T) {
		...
		os.Setenv("KUBEWATCHER_KUBECONFIG", "/tmp/kubeconfig")
		os.Setenv("KUBEWATCHER_IN_CLUSTER", "true")
...
// clearEnv removes all KUBEWATCHER_* and related env vars that could affect config.
func clearEnv(t testing.TB) {
	t.Helper()
	vars := []string{
		"KUBEWATCHER_CONTEXT", "KUBEWATCHER_KUBECONFIG", "KUBECONFIG",
		"KUBEWATCHER_NAMESPACE", "KUBEWATCHER_IN_CLUSTER",
		"DEEPSEEK_API_KEY", "ANOMSTACK_URL", "ANOMSTACK_API_KEY",
		"VERTEX_PROJECT", "VERTEX_LOCATION", "PROMETHEUS_URL",
		"KUBEWATCHER_POPEYE_BIN",
		"ARGOCD_URL", "ARGOCD_TOKEN", "ARGOCD_INSECURE",
		"SNYK_TOKEN", "KUBEWATCHER_TRIVY_BIN", "KUBEWATCHER_FALCO_URL",
		"KUBEWATCHER_TIER", "KUBEWATCHER_LICENSE",
		"KUBEWATCHER_PORT", "KUBEWATCHER_METRICS_PORT",
		"KUBEWATCHER_LOG_LEVEL", "KUBEWATCHER_LOG_FORMAT",
		"KUBEWATCHER_DECISION_LOG",
		"KUBEWATCHER_S3_BUCKET", "KUBEWATCHER_S3_REGION",
		"KUBEWATCHER_S3_ENDPOINT", "KUBEWATCHER_S3_ACCESS_KEY", "KUBEWATCHER_S3_SECRET_KEY",
	}
	for _, v := range vars {
		os.Unsetenv(v)
	}
